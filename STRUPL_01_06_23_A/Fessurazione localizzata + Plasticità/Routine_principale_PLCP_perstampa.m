...
coeff_h = 0.01 ; %percentuale resistanze attivazione hardening
...
%Cracks data
load('Beta.txt');
%Beta contiene le normali associate ai diversi nodi delle diverse fessure
load('Cracking_node.txt');
%Cp contiene i nodi delle diverse fessure ordinati secondo la
%direzione di fessura; se un nodo appartiene a più fessure, il numero di
%tale nodo viene ripetuto.
Cp = Cracking_node' ;
load('Cracking_node_distribution.txt');
%Cpp suddivide i nodi di Cp secondo le diverse direzioni di fessura.
Cpp = Cracking_node_distribution ;
load('Boundary_element.txt');
%Be elenca tutti gli elementi di contorno relativi alle 
%diverse fessure (per ogni direzione solo gli elementi che giacciono da una 
%parte delle fessura)
Be = Boundary_element' ;
load('Boundary_element_distribution.txt');
%Bee suddivide gli elementi di Be secondo le diverse direzioni di fessura.
...
for ii = 1 : step % numero step da scegliere di volta in volta
    ...
    elseif  T(1,cc) >= 10000 % l'ultimo modo attivato è PhiA(>10000)
        ...
        if T(rr,1)< 0 && T(rr,1)>-10000
         ...
         %Se un LambdaB vuole uscire dalla base,seleziono altra riga pivot.
            check_LambdaB = 0 ;  
         %Il check mi serve per interrompere il ciclo quando necessario
...
%                 p = max(Alpha(Alpha<0)) ;
    ...
    %----------------------------------------------------------------------
        ...
        if ii == 1
            ...
            if Tcheck(ii) ~= 2
                Tp = T ; 
        %Se c'è stata una trasformazione vuota il Tp non viene aggiornato
            end
        end
        clear T
   ...
   % Contributo elastico --------------------------------------------------
    ...
    Fns_p = zeros(nne*ndof, ne) ; 
    %ogni colonna mi da le forze nodali di ogni elemento
            ...
            if (ccc >= Cpp(bbb,1)) && (ccc <= Cpp(bbb,2))
                for dd = Bee(bbb,1) : Bee(bbb,2)
                    for iii = 1 : nne
                        ...
                        ww = Be(dd) ;
                        if Cp(ccc) == CC
                            %amplifico lambda per il plot
                            amp = 20 ;
                            d(ww,2*iii-1) = d(ww,2*iii-1)  - amp* Lambdax(ccc) ; 
                            d(ww,2*iii) = d(ww,2*iii) - amp*Lambday(ccc)  ;
                        end
                    end
                end
            end
